    ...
    <div style={{ border: '1px solid #ccc', padding: '1rem', marginTop: '1rem' }}>
      ...
      {problemDetails && (
        <div style={{ color: 'red', border: '1px solid red', padding: '1rem', backgroundColor: '#fff0f0' }}>
          <h4>Error: {problemDetails.title} (Status: {problemDetails.status})</h4>
    ...
    <div style={{ padding: '2rem' }}>
      ...
      <div style={{ marginBottom: '1rem' }}>
        <button onClick={handleCreateUser} disabled={creating}>
          {creating ? 'Creating...' : 'Add Random User'}
        </button>
      </div>
      ...
      {problemDetails && (
        <p style={{ color: 'red' }}>
          Error loading list: {problemDetails.title}
        </p>
      )}

      {users && (
        <ul style={{ textAlign: 'left', listStyle: 'none', padding: 0 }}>
          {/* Show optimistic users first */}
          {optimisticUsers.map((user) => (
            <li key={user.id} style={{ marginBottom: '0.5rem', padding: '0.5rem', border: '1px solid #eee', opacity: 0.6 }}>
              <span style={{ marginRight: '1rem' }}>{user.name} (ID: {user.id})</span>
          ...
          {users
            ...
            .map((user) => (
              <li key={user.id} style={{ marginBottom: '0.5rem', padding: '0.5rem', border: '1px solid #eee' }}>
                ...
                <button onClick={() => handleDeleteUser(user.id)} disabled={deleting} style={{ backgroundColor: '#ff4444', color: 'white' }}>
